 ...
 <section class="container lg:!px-0">
  <div class="flex gap-[2.04rem] lg:flex-row flex-col">
    <div class="w-full flex-auto lg:w-[40%] lg:max-w-[30.47rem] py-[7rem] tab:py-4">
      <h3 class="text-[#254035] text-[4rem] leading-[4rem] font-EBGaramond600 font-[600] mob3:text-[2.5rem] mob3:leading-[3rem] md:text-[40px] heading lg:leading-[52.263px] text-3xl">
        Hear directly from our satisfied clients and partners
      </h3>
    </div>
    <div class="lg:w-[67%] relative py-[7rem] flex-1">
      <div class="bg-[#ECFAFC] absolute h-full w-[90%] top-0 left-[50%] translate-x-[-50%] !z-[-1]"></div>
      <div class="!z-[99] flex overflow-x-auto gap-[3rem] hide-scrollbar px-4">
        <article 
        class="flex flex-col p-[3rem] bg-white shrink-0 lg:basis-[70%] basis-[50%] msgTab:basis-full rounded-[0.63869rem] shadow-xl shadow-[#00000033]"
        v-for="item in comments"
        :key="item.id"
        >
          <div class="flex flex-col gap-[0.62rem] mb-[2.99rem]">
            <h3 class="text-[#2F929C] font-Satoshi500 text-[1.25rem] leading-[1.85rem] tab:text-[1rem]">{{ item.header }}</h3>
            <starsIcon />
          </div>
          <div>
            <h3 class="text-[#000000] font-Satoshi400 text-[1.25rem] leading-[1.93rem] !mb-[2.94rem] tab:text-[1rem]">
              {{ item.body }}
            </h3>
            <div class="w-full bg-[#000000] border border-[#000000] !mb-[2.49rem]"></div>
